...
- The control unit (CU for short) is a part of the CPU that controls the operations of the computer's instructions, especially in relation to the connection between the CPU and the rest of the machine. The CU contains an decoder, which coverts instructions in the CU into languages that can be understood by other devices in the CPU.
